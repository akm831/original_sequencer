...
void prototype_initialize(void* handle, double sample_rate, uint32_t max_callback_frames) {
    if (handle != nullptr) asEngine(handle)->core.initialize(sample_rate, max_callback_frames);
}
...
PrototypeDiagnostics prototype_get_diagnostics(void* handle) {
    if (handle == nullptr) return PrototypeDiagnostics{};
    ...
    return PrototypeDiagnostics{
        ...
        snapshot.callbackStartFrame,
        snapshot.callbackDurationUs,
        snapshot.callbackLoad,
        snapshot.callbackLoadPeak,
